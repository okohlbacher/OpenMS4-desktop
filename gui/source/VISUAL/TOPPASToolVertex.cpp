...
namespace OpenMS
{
  ...
  bool TOPPASToolVertex::initParam_(const QString& old_ini_file)
  {
    ...
    QString program = toQString(ToolHandler::findExecutable(name_));
  ...
  void TOPPASToolVertex::run()
  {
    ...
    for (int round = 0; round < round_total_; ++round)
    {
      ...
      toolScheduledSlot();
      ts->enqueueProcess(TOPPASScene::TOPPProcess(p, toQString(ToolHandler::findExecutable(name_)), args, this));
    }
